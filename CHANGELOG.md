...
### 1. 고품질 TTS 연동 (Google Cloud TTS)
- **목표**: 구독자 전용 고품질 발음 제공
- **구현 내용**:
  - 발음 미리듣기 버튼 추가 (🔊)
  - 무료 회원: 브라우저 내장 TTS (현재)
  - 구독자: Google Cloud TTS (고품질)
  - 오디오 캐싱 시스템 (비용 90% 절감)
- **필요 사항**:
  - Google Cloud API 키 발급
  - 오디오 저장소 (Vercel Blob 또는 AWS S3)
- **예상 비용**: 100만자당 $4~$16
...
## 2026-02-01 - 관리자 대시보드 완성 + 사용자 동기화

### 사용자 동기화 시스템
- **UserSyncProvider**: 로그인 시 자동으로 DB 동기화
- **Clerk 사용자 동기화**: `/api/admin/sync-users` 관리자 API
- 기존 사용자 200 크레딧 자동 지급

### 관리자 권한 수정
- Clerk API로 `publicMetadata.role` 직접 확인
- `checkAdmin()` 함수 개선

### 이용 현황 차트 완성 (`/admin/analytics`)
- 일일 활성 사용자 (라인 차트)
- AI 기능별 사용량 (파이 차트)
- 신규 가입 추이 (바 차트)
- 연습 유형별 분포 (파이 차트)
